...
docker build srcs/images/ftps -t ftps > /dev/null
RESULT=$?
if [ $RESULT -eq 0 ]; then
  echo "\e[32mFTPS is built ✅\e[39m"
else
...
docker build srcs/images/grafana -t grafana > /dev/null
RESULT=$?
if [ $RESULT -eq 0 ]; then
  echo "\e[32mGrafana is built ✅\e[39m"
else
  echo "\e[31Building Grafana failed ❌\e[39m"
fi
...
kubectl apply -f srcs/deployment/grafana/grafana.yaml > /dev/null
